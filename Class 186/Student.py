...
firebase = firebase.FirebaseApplication("https://encryption-and-decryptio-e5823-default-rtdb.firebaseio.com/", None)
login_window = Tk()
...
def getData():
    ...
    get_your_friends = firebase.get("/", your_friends_code)
    if (get_your_friends != None):
        print(get_your_friends)
        bytes_str = bytes.fromhex(get_your_friends)
        original = decrypt("AIM", bytes_str)
        print("original ", original)
        final_message = original.decode("utf-8")
